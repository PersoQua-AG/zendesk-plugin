...
import { listArticles, getArticle, searchArticles, createArticle, updateArticle, createArticleTranslation, updateArticleTranslation, } from '../tools/guide/articles.js';
...
export function registerGuideTools(server, ctx) {
    const { httpClient, cache, securityLevel, markdownDefault } = ctx;
    server.registerTool('zendesk_list_articles', { description: 'List Help Center articles (cursor-paginated, screened).', inputSchema: { pageSize: pageSizeSchema, maxRecords: listMaxRecordsSchema } }, async (args) => okWithHandle(await listArticles(httpClient, cache, args, securityLevel)));
    server.registerTool('zendesk_get_article', { description: 'Get one Help Center article by id (screened).', inputSchema: { articleId: idSchema } }, async ({ articleId }) => okWithHandle(await getArticle(httpClient, cache, { articleId }, securityLevel)));
    server.registerTool('zendesk_search_articles', {
    ...
    server.registerTool('zendesk_create_article', {
    ...
    server.registerTool('zendesk_update_article', {
    ...
    server.registerTool('zendesk_create_article_translation', {
    ...
    server.registerTool('zendesk_update_article_translation', {
    ...
    server.registerTool('zendesk_list_sections', { description: 'List Help Center sections (cursor-paginated, screened).', inputSchema: { pageSize: pageSizeSchema, maxRecords: listMaxRecordsSchema } }, async (args) => okWithHandle(await listSections(httpClient, cache, args, securityLevel)));
    server.registerTool('zendesk_list_categories', { description: 'List Help Center categories (cursor-paginated, screened).', inputSchema: { pageSize: pageSizeSchema, maxRecords: listMaxRecordsSchema } }, async (args) => okWithHandle(await listCategories(httpClient, cache, args, securityLevel)));
    server.registerTool('zendesk_create_section', {
    ...
    server.registerTool('zendesk_create_category', {
