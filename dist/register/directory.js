...
export function registerDirectoryTools(server, ctx) {
    const { httpClient, cache, securityLevel } = ctx;
    server.registerTool('zendesk_search_users', {
    ...
    server.registerTool('zendesk_get_user', { description: 'Get one user by id (screened).', inputSchema: { userId: z.number().int().positive() } }, async ({ userId }) => okWithHandle(await getUser(httpClient, cache, { userId }, securityLevel)));
    server.registerTool('zendesk_upsert_user', {
    ...
    server.registerTool('zendesk_update_user', {
    ...
    server.registerTool('zendesk_list_user_identities', { description: 'List a user’s identities (email/phone), cursor-paginated and screened.', inputSchema: { userId: z.number().int().positive(), pageSize: pageSizeSchema, maxRecords: listMaxRecordsSchema } }, async (args) => okWithHandle(await listUserIdentities(httpClient, cache, args, securityLevel)));
    server.registerTool('zendesk_list_orgs', {
    ...
    server.registerTool('zendesk_get_org', { description: 'Get one organization by id (screened).', inputSchema: { orgId: z.number().int().positive() } }, async ({ orgId }) => okWithHandle(await getOrg(httpClient, cache, { orgId }, securityLevel)));
    server.registerTool('zendesk_upsert_org', {
    ...
    server.registerTool('zendesk_update_org', {
    ...
    server.registerTool('zendesk_list_org_memberships', { description: 'List organization memberships (user↔org links), cursor-paginated.', inputSchema: { pageSize: pageSizeSchema, maxRecords: membershipMaxRecordsSchema } }, async (args) => okWithHandle(await listOrgMemberships(httpClient, cache, args, securityLevel)));
    server.registerTool('zendesk_list_groups', { description: 'List agent groups (cursor-paginated, screened).', inputSchema: { pageSize: pageSizeSchema, maxRecords: listMaxRecordsSchema } }, async (args) => okWithHandle(await listGroups(httpClient, cache, args, securityLevel)));
    server.registerTool('zendesk_list_group_memberships', { description: 'List group memberships (user↔group links), cursor-paginated.', inputSchema: { pageSize: pageSizeSchema, maxRecords: membershipMaxRecordsSchema } }, async (args) => okWithHandle(await listGroupMemberships(httpClient, cache, args, securityLevel)));
}
