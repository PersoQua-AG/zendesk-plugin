...
export async function authorize(deps) {
    const { config, tokensPath, waitForCode = waitForAuthorizationCode, exchange = exchangeCodeForTokens, generateVerifier = generateCodeVerifier, generateState = () => randomBytes(16).toString('base64url'), now = Date.now, print = (line) => process.stdout.write(`${line}\n`), } = deps;
