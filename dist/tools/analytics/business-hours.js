...
export function localDate(timeZone, epochMs) {
    const dtf = new Intl.DateTimeFormat('en-US', {
        timeZone,
        hourCycle: 'h23',
...
function parseHm(hm) {
    ...
    if (!m)
        throw new Error(`Invalid work-hours time "${hm}" — expected 24h "HH:MM".`);
    return { hour: Number(m[1]), minute: Number(m[2]) };
}
export function calendarMinutesBetween(startMs, endMs) {
    if (endMs <= startMs)
        return 0;
...
export function businessMinutesBetween(startMs, endMs, config) {
    ...
    let totalMs = 0;
    let cursor = localDate(config.timeZone, startMs);
...
export function parseReportConfig(env) {
