...
export async function listTicketFields(client, cache) {
    const raw = await client.request('/ticket_fields.json');
    const parsed = FieldsSchema.safeParse(raw);
    if (!parsed.success)
        throw new Error('Unexpected /ticket_fields response shape.');
...
export async function listTicketForms(client, cache) {
    try {
        const raw = await client.request('/ticket_forms.json');
        const parsed = FormsSchema.safeParse(raw);
        if (!parsed.success)
            throw new Error('Unexpected /ticket_forms response shape.');
