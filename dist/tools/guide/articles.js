...
function assertLocaleShape(locale) {
    if (!LOCALE_SHAPE.test(locale))
        throw new Error(`Invalid translation locale "${locale}" — expected a shape like "en-us" or "de".`);
}
...
const describeArticle = makeDescribe('article', (a) => `#${a.id} ${a.title ?? '(untitled)'}${a.draft ? ' (draft)' : ''} [${a.locale ?? '?'}]`);
...
export async function listArticles(client, cache, params = {}, securityLevel = 'standard') {
    return listCbp({
        client,
        cache,
        securityLevel,
...
export async function getArticle(client, cache, params, securityLevel = 'standard') {
    const raw = await client.request(`/help_center/articles/${params.articleId}.json`);
    const parsed = SingleArticleSchema.safeParse(raw);
    if (!parsed.success)
        throw new Error('Unexpected /help_center/articles/{id} response shape.');
...
export async function searchArticles(client, cache, params, securityLevel = 'standard') {
    if (params.query.trim() === '')
        throw new Error('search_articles requires a non-empty query.');
    ...
    if (!parsed.success)
        throw new Error('Unexpected /help_center/articles/search response shape.');
...
export function createArticle(client, cache, params, securityLevel = 'standard') {
    ...
    return createEntity(client, cache, { collection: `/help_center/sections/${params.sectionId}/articles`, key: 'article', toolName: 'zendesk_create_article', resourceLabel: 'article', requiredFields: ['title', 'locale', 'body'], guard: withAdminGuard }, built, securityLevel);
}
export function updateArticle(client, cache, params, securityLevel = 'standard') {
    ...
    return updateEntity(client, cache, { collection: '/help_center/articles', key: 'article', toolName: 'zendesk_update_article', resourceLabel: 'article', guard: withAdminGuard }, params.articleId, built, securityLevel);
}
export function createArticleTranslation(client, cache, params, securityLevel = 'standard') {
    const locale = params.fields.locale ?? DEFAULT_LOCALE;
    assertLocaleShape(locale);
    const body = renderRequiredBody(params.fields.body, params.markdown);
    const built = stripUndefined({ ...params.fields, locale, body });
    return createEntity(client, cache, { collection: `/help_center/articles/${params.articleId}/translations`, key: 'translation', toolName: 'zendesk_create_article_translation', resourceLabel: 'article translation', requiredFields: ['locale', 'title', 'body'], guard: withAdminGuard }, built, securityLevel);
}
export function updateArticleTranslation(client, cache, params, securityLevel = 'standard') {
    ...
    return updateEntity(client, cache, { collection: `/help_center/articles/${params.articleId}/translations`, key: 'translation', toolName: 'zendesk_update_article_translation', resourceLabel: 'article translation', guard: withAdminGuard }, params.locale, built, securityLevel);
}
