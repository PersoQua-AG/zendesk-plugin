...
export const CONNECTOR_AUTHORIZE_HINT =
  'Authorize the Zendesk connector (complete the OAuth sign-in) and try again.';
...
export function describeAuthError(err: unknown): string {
  ...
  if (/re-authorize|No Zendesk authorization|requires an admin role/i.test(msg)) return msg;
  if (/unauthorized|invalid_token|missing a zendesk identity/i.test(msg)) return CONNECTOR_AUTHORIZE_HINT;
  return msg;
}
