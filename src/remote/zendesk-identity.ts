...
export async function fetchZendeskIdentity(
  subdomain: string,
  accessToken: string,
  ...
  if (!res.ok) throw new Error(`Failed to resolve Zendesk identity (HTTP ${res.status}).`);
  const parsed = meSchema.safeParse(await res.json());
  if (!parsed.success) throw new Error('Malformed /users/me response — cannot resolve identity.');
