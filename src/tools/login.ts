...
// The one authorization in progress. There is exactly one user on the stdio path, so a second
// concurrent flow could only ever be the model calling twice — and that call must COLLECT this
...
type FlowOutcome =
  | { kind: 'pending' }
  | { kind: 'received'; result: AuthorizationResult }
  // Already rendered for the user: the listener rejects with a reason, and turning it into text
  // right there keeps the redaction rules in one place.
