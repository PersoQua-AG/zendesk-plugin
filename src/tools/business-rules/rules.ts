...
const describeTrigger = makeDescribe<Rule>('trigger', (t) => `#${t.id} ${t.title ?? '(untitled)'}${t.active === false ? ' (inactive)' : ''}`);
const describeAutomation = makeDescribe<Rule>('automation', (a) => `#${a.id} ${a.title ?? '(untitled)'}${a.active === false ? ' (inactive)' : ''}`);
...
export type RuleWriteFields = z.infer<typeof ruleWriteFieldsSchema>;
...
const AUTOMATION_WRITE: Omit<RuleWriteConfig, 'toolName'> = { collection: '/automations', key: 'automation', resourceLabel: 'automation', requiredFields: ['title'] };
...
const SLA_WRITE: Omit<RuleWriteConfig, 'toolName'> = { collection: '/slas/policies', key: 'sla_policy', resourceLabel: 'sla-policy', requiredFields: ['title'] };
